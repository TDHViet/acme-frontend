...
import { Mail, Lock, Eye, EyeOff, User, CircleX } from "lucide-react"
...
const schema = z.object({
  ...
  password: z.string().min(8, "8 or more characters"),
})
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-card rounded-lg border border-border p-8 space-y-6">
          <div className="space-y-2 text-center">
            ...
            <p className="text-sm text-muted-foreground">Please fill in the details to get started.</p>
          </div>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              <Label htmlFor="name" className="text-card-foreground font-semibold">Name</Label>
              <div className="relative mt-1">
                <User className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                <Controller
                  control={control}
                  name="name"
                  render={({ field }) => <Input id="name" className="pl-9" {...field} />}
                />
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
                <CircleX className="h-4 w-4" /> 8 or more characters
              </p>
            </div>
          ...
          <p className="text-center text-sm text-muted-foreground">
            Already have an account? <Link to="/auth/sign-in" className="text-card-foreground underline">Sign in</Link>
          </p>
        </div>
      </div>
      <AuthThemeSwitcher />
